...
class SideBar extends Component {
  constructor(props) {
    ...
    this.updateValue = event => {
      this.setState(
        { searchValue: event.target.value, selectionId: "" },
    ...
    this.filterRestaurants = () => {
      ...
      if (this.state.searchValue !== "") {
        filtered = this.props.restaurants.filter(object =>
          match.test(object.restaurant.name)
        );
      } else {
  ...
  UNSAFE_componentWillReceiveProps() {
    ...
    if (!this.state.filtered.length) {
      ...
      this.state.filtered[0].restaurant.placeholder &&
  ...
  render() {
    return (
      <nav
        className={
          this.props.navIsShown ? this.styles.open : this.styles.closed
        }
      >
        ...
        <section className="flex-container column full-width search-box-container">
          <label htmlFor="searchBox" className="search-box-label">
            Search Restaurants:
          </label>
          <input
            type="text"
            id="searchBox"
            placeholder="e.g. Texas Hot"
        ...
        <ul className="restaurants-list" aria-label="restaurant search results">
          {this.state.filtered.map(object => (
            <ListItem
              name={object.restaurant.name}
              key={object.restaurant.id}
...
class ListItem extends Component {
  render() {
    return (
      <li className="restaurant-li flex-container">
        <a
          ref="link"
